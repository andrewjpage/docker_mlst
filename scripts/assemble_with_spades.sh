...
for FORWARD_FILE in $(find ${HOST_BASE} -type f -name "*_1.fastq.gz");
  do
    ...
    { time docker run --rm -v ${HOST_BASE}:/data ${DOCKER_HASH} spades.py -t 1 -1 ${FORWARD_FILE} -2 ${REVERSE_FILE} -o ${OUTPUT_DIRECTORY}/${BASE_NAME} --phred-offset 33  > ${OUTPUT_DIRECTORY}/output_${BASE_NAME} ; }  2> ${HOST_BASE}/fastas/timings_${BASE_NAME}
done
